...
class SoundBank:
    ...
    def _build_all(self):
        ...
        recipes = {
            ...
            # ポン：パドルでボールを打ち返す音
            "paddle_hit": ([(300, 500, 0.045)], "square", 0.24),
            # コツ：壁・天井で跳ね返る軽い音
            "wall_hit": ([(500, 400, 0.035)], "square", 0.18),
            # パリン：ブロックが割れる音
            "brick_break": ([(900, 300, 0.07)], "square", 0.24),
            # ピュン：自機弾を発射する短い下降音
            ...
            # パチンッ：フリッパー作動の短いクリック
            "flipper": ([(500, 700, 0.03)], "square", 0.18),
            # ボヨンッ：バンパー命中の弾む音
            "bumper": ([(700, 1100, 0.04), (500, 300, 0.05)], "square", 0.26),
            # コンッ：スタンドターゲット命中の短い高音
            "target": ([(1000, 1000, 0.05)], "square", 0.22),
            # シュイーン：ランプ通過の上昇スイープ
            "ramp": ([(500, 1400, 0.16)], "triangle", 0.26),
            # ヒュルルル：ループレーン通過の長めの上昇スイープ
            "loop": ([(400, 900, 0.1), (900, 1500, 0.14)], "triangle", 0.26),
            # ジャジャーン：マルチボール開始のファンファーレ
            "multiball": ([(660, 660, 0.08), (880, 880, 0.08),
                           (1100, 1100, 0.08), (1320, 1320, 0.22)], "square", 0.28),
            # パパパーン：ジャックポット獲得の派手な上昇音
            "jackpot": ([(880, 880, 0.06), (1100, 1100, 0.06),
                        (1320, 1320, 0.06), (1760, 1760, 0.26)], "square", 0.3),
            # トンッ：発射（プランジャー）の短い低音
